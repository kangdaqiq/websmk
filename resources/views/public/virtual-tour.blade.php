...
@push('styles')
    ...
    <style>
        ...
        .pnlm-hotspot-base.nav-hotspot span {
        ...
        .pnlm-hotspot.nav-hotspot {
            background: rgba(79, 70, 229, .9) !important;
            border: 3px solid #fff !important;
            border-radius: 50% !important;
            width: 40px !important;
            height: 40px !important;
            box-shadow: 0 0 0 5px rgba(99, 102, 241, .4), 0 4px 12px rgba(0, 0, 0, .4) !important;
            animation: pulse-ring 1.8s ease infinite !important;
        }

        .pnlm-hotspot.nav-hotspot:hover {
        ...
        @keyframes pulse-ring {
            ...
            0%,
            100% {
                box-shadow: 0 0 0 5px rgba(99, 102, 241, .4), 0 4px 12px rgba(0, 0, 0, .4);
            }

            50% {
                box-shadow: 0 0 0 10px rgba(99, 102, 241, .15), 0 4px 12px rgba(0, 0, 0, .4);
            }
        }
    </style>
@endpush
    ...
    <div class="bg-gray-50 min-h-screen">
        ...
        <div class="bg-gradient-to-br from-indigo-900 via-purple-900 to-indigo-800 text-white py-10 px-4">
            <div class="max-w-7xl mx-auto text-center">
                <span
                    class="inline-block bg-white/10 border border-white/20 text-xs font-bold uppercase tracking-widest px-4 py-1 rounded-full mb-3 backdrop-blur-sm">
                    🌐 360° Virtual Tour
                </span>
            ...
            <div class="bg-white rounded-2xl shadow-xl p-4 mb-6">
                ...
                <div class="relative">
                    ...
                    <div id="debug-panel">
                        ...
                        <span class="text-xs opacity-60 mt-1 block">Arahkan mouse ke posisi<br>yang ingin dijadikan
                            hotspot</span>
                    </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-8 gap-3" id="scene-map">
                    @foreach($rooms as $i => $room)
                        <div class="scene-btn unavailable bg-gray-50 rounded-xl p-2 text-center"
                            id="scene-btn-{{ $room['id'] }}" data-id="{{ $room['id'] }}" data-title="{{ $room['title'] }}"
                            data-cat="{{ $room['category'] }}" onclick="goToScene('{{ $room['id'] }}')">
    ...
    <script>
        ...
        function loadScene(sceneId) {
            ...
            const hotspots = (HOTSPOTS[sceneId] || []).map(h => ({
                ...
                clickHandlerFunc: function (evt, args) { loadScene(args.targetScene); },
                clickHandlerArgs: { targetScene: h.targetScene },
            }));
